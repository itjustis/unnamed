...
def tile_upscale(seed,image_path,upr,prompt,negative,pipe,controlnets,cn_scales,tile_size=768, shift=0.333,steps=25,scale=7.5,strength=0.666,interrogate=False):
	img_upscaled , original_size , upscaled_size= upscale_image(image_path,upr)
	img_upscaled = img_upscaled.convert('RGB')
	result = process_tiles(seed, pipe, controlnets, cn_scales, img_upscaled, original_size, prompt, negative, strength, tile_size, shift,steps,scale,interrogate)
	return result
...
def color_match(prev_img, color_match_sample):
	...
	# Apply Gaussian blur to smooth the harsh transitions
	smoothed_matched_rgb = cv2.GaussianBlur(matched_rgb, (5, 5), 0)
	
	return cv2.cvtColor(smoothed_matched_rgb, cv2.COLOR_BGR2RGB)
